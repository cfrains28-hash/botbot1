...
if not df_ui.empty:
    ...
    if show_vp:
        fig = make_subplots(rows=2, cols=2, shared_xaxes=True, shared_yaxes=True, column_widths=[0.2, 0.8], row_heights=[0.8, 0.2], specs=[[{}, {}], [{}, {}]], horizontal_spacing=0.01, vertical_spacing=0.05)
        fig.add_trace(go.Bar(x=u_vp['volume'], y=u_vp['mid'], orientation='h', marker=dict(color='gray', opacity=0.3), name='매물대'), row=1, col=1)
        c_col = 2
    else:
